# 💜 Olá, meu nome é Vitor!

Busco desenvolver minha carreira na área de Tecnologia da Informação, com foco em **Cloud Computing e Infraestrutura**, atuando na criação, automação e gerenciamento de ambientes em nuvem.

Sou graduando em Sistemas de Informação, com experiência em **suporte nível 2**, e atualmente focado em evolução para a área de **Cloud (AWS)** e **Linux**. Possuo conhecimentos em **SQL, ETL, AWS e boas práticas de ITSM**, além de estar constantemente aprimorando minhas habilidades em automação e infraestrutura como código.

Procuro oportunidades que proporcionem aprendizado contínuo, desafios técnicos e aplicação prática do conhecimento, contribuindo de forma eficiente, analítica e orientada a resultados.

---

## 💬 Vamos nos conectar?

Fique à vontade para entrar em contato comigo!  
Se tiver dúvidas, oportunidades ou quiser trocar uma ideia sobre tecnologia, será um prazer conversar 🚀

---

## 🚀 Minhas Skills

### 💻 Linguagens & Tecnologias
![MySQL](https://img.shields.io/badge/MySQL-000?style=for-the-badge&logo=mysql)
![JavaScript](https://img.shields.io/badge/JavaScript-000?style=for-the-badge&logo=javascript)
![HTML](https://img.shields.io/badge/HTML-000?style=for-the-badge&logo=html5)
![CSS](https://img.shields.io/badge/CSS-000?style=for-the-badge&logo=css3)
![Arduino](https://img.shields.io/badge/Arduino-000?style=for-the-badge&logo=arduino)
![React](https://img.shields.io/badge/React-000?style=for-the-badge&logo=react)

---

### 🛠️ Utilidades
![Postman](https://img.shields.io/badge/Postman-000?style=for-the-badge&logo=postman)
![AWS](https://img.shields.io/badge/AWS-000?style=for-the-badge&logo=amazonaws)

---

### ⚙️ DevOps
![Git](https://img.shields.io/badge/Git-000?style=for-the-badge&logo=git)
![GitHub](https://img.shields.io/badge/GitHub-000?style=for-the-badge&logo=github)
![Docker](https://img.shields.io/badge/Docker-000?style=for-the-badge&logo=docker)

---

### 🧰 Ferramentas de Desenvolvimento
![VSCode](https://img.shields.io/badge/VSCode-000?style=for-the-badge&logo=visualstudiocode)
![Trello](https://img.shields.io/badge/Trello-000?style=for-the-badge&logo=trello)
![Figma](https://img.shields.io/badge/Figma-000?style=for-the-badge&logo=figma)

---

## ⭐ GitHub Stats

![Vitor's GitHub stats](https://github-readme-stats.vercel.app/api?username=SEU_USERNAME&show_icons=true&theme=dark)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=SEU_USERNAME&layout=compact&theme=dark)

